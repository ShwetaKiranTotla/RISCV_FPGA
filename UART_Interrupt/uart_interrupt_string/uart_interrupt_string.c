...
#include "log.h"
#include "uart.h"
//This program is to send a string and receive using interrupt
...
//Characters are shown once every 10 times the interrupt is receive	memset(u2_rcv_char,'\0', UARTX_BUFFER_SIZE);
...
*/
	...
	write_uart_string(uart_instance[1], "\n-----------UART 0: Transmitting characters received-----------\n");
        ...
        //write_uart_string(uart_instance[2],"Hello World");
	while(1)
	{
		write_uart_string(uart_instance[2],"Hello World");
